...
function openReviewModal(profId){
...
modal.addEventListener('click', (e)=>{ if (e.target.matches('[data-action="close"], .modal-backdrop')) closeModal(); });
document.getElementById('form-review').addEventListener('submit', async (e)=>{
  ...
  msg.textContent = 'Enviando...';
  try{
    const res = await fetch(`${API}/professionals/${profId}/reviews`,{ method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ user, stars, comment }) });
    if(!res.ok) throw new Error('error');
    await res.json();
