...
function renderProfessionals(items) {
  ...
  items.forEach(p => {
    ...
    // Botones acciones añadidos debajo
    const actions = document.createElement('div');
    actions.style.marginTop = '8px';
    const btnVideo = document.createElement('button');
    btnVideo.className = 'btn';
    btnVideo.textContent = 'Ver video';
    btnVideo.addEventListener('click', () => {
      location.href = `video.html?professionalId=${encodeURIComponent(p.id)}`;
    });
    const btnClass = document.createElement('button');
    btnClass.className = 'btn';
    btnClass.style.marginLeft = '8px';
    btnClass.textContent = 'Tomar clase';
    btnClass.addEventListener('click', async () => {
      // Ir directo al checkout simulado con 1 clase
      location.href = `video.html?professionalId=${encodeURIComponent(p.id)}&cta=pay`;
    });
    actions.appendChild(btnVideo);
    actions.appendChild(btnClass);
